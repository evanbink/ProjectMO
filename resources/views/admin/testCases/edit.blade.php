...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.testCase.title_singular') }}
    </div>

    <div class="card-body">
        <form action="{{ route("admin.test-cases.update", [$testCase->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                ...
                <select id="status" name="status" class="form-control" required>
                    ...
                    @foreach(App\TestCase::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $testCase->status) === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
